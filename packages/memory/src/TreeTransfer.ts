 ...
 * contents, so entries carry the source's original access time. The reads
 * themselves refresh source access times under relatime, so only a read whose
 * access time is due changes the source, and on a durable volume only such a
 * read commits. Use
 ...
 * special mode bits follow {@link VolumeTransferOptions}. Every entry is held in
