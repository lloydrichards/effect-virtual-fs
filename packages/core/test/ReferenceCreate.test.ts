...
describe("conditional child creation", () => {
  it.effect("rejects a newly present or replaced child before truncating it", () =>
    Effect.gen(function*() {
      ...
      const first = yield* caller.openChildReference(root, name("file"), {
        access: "readWrite",
        create: "exclusive",
        expectedChild: null
      })
      ...
      const missing = yield* Effect.flip(caller.openChildReference(root, name("file"), {
        access: "write",
        create: "ifMissing",
        truncate: true,
        expectedChild: null
      ...
      yield* caller.unlinkReference(root, name("file"))
      yield* caller.writeFile("file", new Uint8Array([2, 3]), { access: "write", create: "exclusive" })

      const replaced = yield* Effect.flip(caller.openChildReference(root, name("file"), {
        access: "write",
        truncate: true,
        expectedChild
      }))
  ...
  it.effect("checks both mutation revisions and access times before opening", () =>
    Effect.gen(function*() {
      ...
      const expectedChild = yield* observeChild(caller, first.reference)
      const matching = yield* caller.openChildReference(root, name("file"), { access: "read", expectedChild })
      assert.strictEqual(matching.reference, first.reference)
      yield* caller.readFile("file")
      assert.strictEqual((yield* caller.observeMetadata(first.reference)).revision, expectedChild.revision)
      assert.strictEqual(
        (yield* Effect.flip(caller.openChildReference(root, name("file"), {
          access: "read",
          expectedChild
        }))).code,
        "StaleReference"
  ...
  it.effect("creates initial size and ownership without changing initial timestamps", () =>
    Effect.gen(function*() {
      ...
      const opened = yield* caller.openChildReference(root, name("file"), {
        access: "readWrite",
        create: "exclusive",
        mode: 0o666,
        initialSize: 3n,
      ...
      const existing = yield* caller.openChildReference(root, name("file"), {
        access: "read",
        create: "ifMissing",
        initialSize: 9n,
        owner: { uid: 9, gid: 9 }
  ...
  it.effect("leaves no entry when initial size exceeds file or volume capacity", () =>
    Effect.gen(function*() {
      for (
        ...
        const error = yield* Effect.flip(caller.openChildReference(root, name("file"), {
          access: "write",
          create: "exclusive",
          initialSize: 3n
        }))
        ...
        assert.deepStrictEqual(yield* caller.observeDirectory(root), before)
        assert.strictEqual((yield* volume.usage).usedBytes, 0n)
      }
    }))
  ...
  it.effect("checks initial ownership before publishing and accepts a caller group", () =>
    Effect.gen(function*() {
      ...
      yield* admin.chmodReference(root, 0o777)
      ...
      for (const owner of [{ uid: 10 }, { gid: 10 }]) {
        ...
        assert.strictEqual((yield* Effect.flip(caller.lookupReference(root, name("file")))).code, "NotFound")
      }
