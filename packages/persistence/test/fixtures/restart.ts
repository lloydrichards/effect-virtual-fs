...
const program = Effect.gen(function*() {
  ...
  const link = yield* caller.stat(Vfs.Target.Path({ path: "/link", followFinalSymlink: false }))
  assert.equal(link.kind, "symlink")
  assert.equal(link.ino, (yield* caller.stat(Vfs.Target.Path({ path: "/link-alias", followFinalSymlink: false }))).ino)
  assert.equal(link.nlink, 2)
  assert.deepEqual(yield* caller.readLink("/link"), new Uint8Array([254, 47, 120]))
