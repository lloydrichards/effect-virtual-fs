...
export const proseComponents = {
  ...
  tr: ({ className, ...props }: React.HTMLAttributes<HTMLTableRowElement>) => (
    <tr
      className={cn("m-0 border-t p-0 first:border-t-0", className)}
      {...props}
    />
  ),
  ...
  code: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => {
    ...
    return (
      <code
        className={cn(
          "break-words rounded-md bg-code text-code-foreground border border-code-border px-1.5 py-0.5 font-mono text-[0.875em] tracking-normal",
          className
        )}
        {...props}
      />
