...
Application code can change the same volume through its VFS caller while NFS clients read it. The server rejects mutating NFS operations with `NFS4ERR_ROFS`. It tracks advisory read locks between NFS clients, but direct VFS callers do not participate in those locks.
