...
```ts
export type SnapshotChangesOptions = { readonly includeTimestamps?: boolean }
```
...
```ts
export type SnapshotDeltaLimits = {
...
```ts
export type SnapshotNodeKind = "directory" | "file" | "symlink"
```
...
```ts
export type SnapshotDeltaTypeId = "@effect-vfs/core/SnapshotDelta"
```
