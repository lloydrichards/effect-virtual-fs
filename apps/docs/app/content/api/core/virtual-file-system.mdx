...
```ts
export declare const fromFixture: (fixture: Fixture, options?: VolumeOptions) => Effect.Effect<Volume, VfsError>
```
...
```ts
export declare const fromSnapshot: (snapshot: Snapshot, options?: VolumeOptions) => Effect.Effect<Volume, VfsError>
```
...
```ts
export declare const make: (options?: VolumeOptions) => Effect.Effect<Volume, VfsError>
```
...
```ts
export declare const makeOverlay: (base: Snapshot, options?: VolumeOptions) => Effect.Effect<OverlayVolume, VfsError>
```
...
```ts
export declare const pathFromBytes: (bytes: Uint8Array) => Effect.Effect<BytePath, FsFailure>
```
...
```ts
export declare const pathToBytes: (path: BytePath) => Effect.Effect<Uint8Array, FsFailure>
```
...
const program = Effect.gen(function* () {
  ...
  yield* caller.writeFile(
    Vfs.Target.Path({ path: "notes.txt", relativeTo: directory }),
...
export interface OverlayVolume extends Volume {
  ...
  readonly changes: (options?: OverlayChangesOptions) => Effect.Effect<ReadonlyArray<OverlayChange>, VfsError>
  /** Captures one committed state when this reusable effect executes. */
  readonly capture: (options?: OverlayChangesOptions) => Effect.Effect<OverlayCapture, VfsError>
}
```
...
export type ReferenceKey = {
  readonly identity: Uint8Array<ArrayBufferLike>
  readonly epoch: Uint8Array<ArrayBufferLike>
  readonly ino: bigint
  readonly tag: Uint8Array<ArrayBufferLike>
}
```

Added in v0.6.0

### RemoveOptions (type alias)
...
```ts
export type RemoveOptions = { readonly recursive?: boolean; readonly force?: boolean }
```
...
const program = Effect.gen(function* () {
  ...
  const watcher = yield* volume
    .watch()
    .pipe(Effect.flatMap(Stream.runHead), Effect.forkChild({ startImmediately: true }))
...
```ts
export declare const ReferenceKey: Schema.Struct<{
  readonly identity: Schema.Uint8ArrayFromBase64
  readonly epoch: Schema.Uint8ArrayFromBase64
  readonly ino: Schema.BigIntFromString
  readonly tag: Schema.Uint8ArrayFromBase64
}>
```
...
```ts
export declare const RemoveOptions: Schema.Struct<{
  readonly recursive: Schema.optionalKey<Schema.Boolean>
...
```ts
export declare const SetattrOptions: Schema.Struct<{
  readonly size: Schema.optionalKey<Schema.BigInt>
  readonly mode: Schema.optionalKey<Schema.Natural>
  readonly owner: Schema.optionalKey<
    Schema.Struct<{
      readonly uid: Schema.optionalKey<Schema.Natural>
  ...
  >
  readonly expected: Schema.optionalKey<Schema.Struct<{ readonly revision: Schema.BigInt }>>
}>
```
...
```ts
export declare const WalkOptions: Schema.Struct<{
  readonly order: Schema.optionalKey<Schema.Literals<readonly ["pre", "post"]>>
  readonly maxDepth: Schema.optionalKey<Schema.Natural>
  readonly maxEntries: Schema.optionalKey<Schema.Natural>
  readonly maxBytes: Schema.optionalKey<Schema.ByteSize>
}>
```
...
The same decoder as `decodeSnapshotSink`, run over one chunk.
...
export declare const decodeSnapshot: (
  input: Uint8Array,
  limits: DecodeLimits
) => Effect.Effect<Snapshot, ImageFailure | ArgumentFailure>
```
...
```ts
export declare const decodeSnapshotSink: (
  limits: DecodeLimits
) => Sink.Sink<Snapshot, Uint8Array, never, ImageFailure | ArgumentFailure>
```
...
The bytes are the chunks of `encodeSnapshotStream` collected into one
...
export declare const encodeSnapshot: (
  snapshot: Snapshot,
  limits?: DecodeLimits
) => Effect.Effect<Uint8Array, ImageFailure | ArgumentFailure>
```
...
```ts
export declare const encodeSnapshotStream: (
  snapshot: Snapshot,
  limits?: DecodeLimits
) => Stream.Stream<Uint8Array, ImageFailure | ArgumentFailure>
```
...
const program = Effect.gen(function* () {
  ...
  const text = yield* Vfs.encodeSnapshotStream(yield* volume.snapshot).pipe(Stream.decodeText, Stream.mkString)
...
```ts
export declare const Caller: Context.Service<Caller, Caller> & {
...
```ts
export declare const Volume: Context.Service<Volume, Volume> & {
...
Effect.runPromise(program.pipe(Effect.provide(Vfs.Caller.layer()), Effect.provide(Vfs.Volume.layer()))).then(
  console.log
)
// [ 'work' ]
```
...
```ts
export declare const snapshotEntries: (
  snapshot: Snapshot,
  root: PathInput
) => Stream.Stream<Fixture["entries"][number], FsFailure | ImageFailure>
```
...
- [Filesystem error types](/api/core/vfs-error)
