...
export declare const make: (
  options?: VolumeOptions,
) => Effect.Effect<Volume, ConfigurationError>;
```
...
export interface Caller {
  ...
  readonly lookupReference: (
    directory: ObjectReference,
    name: Uint8Array,
  ) => Effect.Effect<ObjectReference, FsError>;
  ...
  ) => Effect.Effect<ObjectObservation<Metadata>, FsError>;
