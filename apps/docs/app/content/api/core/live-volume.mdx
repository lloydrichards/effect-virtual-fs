...
export declare const open: (
  options: Options
) => Effect.Effect<Volume, VfsError, LiveImageStore | Crypto.Crypto | Scope.Scope>
```
...
) => Effect.Effect<ImageSession, VfsError | PlatformError.PlatformError, Crypto.Crypto>
```
...
) => Effect.Effect<Uint8Array, VfsError | PlatformError.PlatformError, Crypto.Crypto>
```
