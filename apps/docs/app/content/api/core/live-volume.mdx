...
```ts
export declare const open: (options: Options) => Effect.Effect<Volume, VfsError, LiveImageStore | Scope.Scope>
```
...
) => Effect.Effect<ImageSession, VfsError>
```
...
```ts
export declare const prepareEmptyImage: (options?: VolumeOptions) => Effect.Effect<Uint8Array, VfsError>
```
