...
The three published Effect Virtual FS packages implement a documented subset of POSIX filesystem behavior in memory.
...
| Package                   | Effect peer    | Module format | Purpose                                                            |
| ------------------------- | -------------- | ------------- | ------------------------------------------------------------------ |
| `@effect-vfs/core`        | `4.0.0-rc.114` | ESM only      | Runtime-neutral volumes, callers, handles, snapshots, and overlays |
| `@effect-vfs/memory`      | `4.0.0-rc.114` | ESM only      | Effect `FileSystem.FileSystem` adapter                             |
| `@effect-vfs/persistence` | `4.0.0-rc.114` | ESM only      | Named SQLite snapshot checkpoints                                  |

Use an ES module build and install the exact Effect peer version shown above.
...
These published packages do not claim full POSIX conformance. They do not provide a C filesystem API, host mount,
simulated process, host-directory import or export, device files, FIFOs, filesystem sockets, advisory locks, or
descriptor duplication. Snapshot checkpoints are explicit copies, not automatic persistence of live writes or host
`fsync` durability.
...
- The current implementation uses an unbounded queue and exposes no caller-configurable event or byte limit.
- Closing the owning scope releases the subscription.

A slow consumer can therefore retain queued events in memory. Apply an application-level consumption or lifetime policy
when event volume is not already bounded by the workload.
