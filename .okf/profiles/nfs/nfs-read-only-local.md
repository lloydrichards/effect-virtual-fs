...
`read-only-local` is the first profile in the [NFS profile ladder](../../decisions/nfs/nfs-profile-ladder.md "implements"). It is a protocol-complete read-only NFSv4.1 export of one live volume for a trusted local user. It is not a conformant NFSv4.1 server; RFC 8881 requires Kerberos, which the [authentication and export policy decision](../../decisions/nfs/nfs-authentication-and-export-policy.md "constrained by") excludes permanently.[^rfc8881]
...
[^server]: `NfsServerAddress` accepts the two literal loopback addresses or a UNIX-domain socket path, and limits are validated Effect schemas.
