...
generated: { by: claude/okf, at: 2026-09-19T10:06:14Z }
---
...
- `AUTH_NONE` is a flavor the networked policy can see and decide, exactly like `AUTH_SYS`, when included in `acceptedFlavors`. SECINFO and SECINFO_NO_NAME advertise that configured list in order: `[AUTH_SYS, AUTH_NONE]` for `read-only-local`, `[AUTH_SYS]` by default for `read-only-networked`. The policy can still deny individual credentials in an advertised flavor. No anonymous identity constant lives in the package.

## Identity mapping

- The application supplies a peer resolver Effect evaluated on each accepted socket and a policy function. Its input is the decoded credential (flavor, uid, gid, supplementary groups, machine name) and the peer. TCP peers carry the remote address and port; UNIX peers carry `address: null`, `port: null`, and the server socket path, which is not a client address. A resolver returning `null` closes the connection. Its output is deny or a VFS identity `{ uid, gid, groups, privileged }`. The package ships no client allow-list syntax; peer filtering is the policy's job.
...
[^dispatcher]: `grantedAccess` documents the advisory ACCESS boundary; `principalKey` and `credentialsKey` derive from the raw credential; replay also compares the mapped caller; EXCHANGE_ID rejects `SP4_MACH_CRED` and `SP4_SSV`.

[^server]: `NfsServerAddress` accepts TCP or a UNIX-domain socket path; the runtime gate rejects non-loopback TCP without policy and explicit opt-in.
