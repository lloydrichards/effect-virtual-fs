...
sources:
  ...
  - id: tree-schema
    ...
    title: The one tree schema, its line reader and budget meter, and the converters to and from the value
  - id: step-7-decisions
...
The core engine is rebuilt around one idea: the volume's state is a persistent, immutable tree value, and every operation is a pure transition run by a single pipeline. Staging, overlay, watch, and revisions compose over that pipeline as decorators and layers rather than living inside it. Serialisation collapses to one Schema-defined tree, and the public surface halves behind a target type. The work is staged as ordered refactors with clear goals and designs with open questions, so it can be handed over or taken up step by step. The [sequencing issue](https://github.com/lloydrichards/effect-virtual-fs/issues/189 "discusses") owns the order and handover discussion. Steps 1 to 8 are delivered; step 7 landed in three parts, the value and one validator, then the Merkle identity, change-event deltas and one budget, then the streaming codecs and their consumers.
...
7. Serialisation on one tree schema, in three drafts, all delivered: the value and one validator (snapshots over the value, the internal tree and its converters, the live tree, and the fixture fold); the Merkle identity, change-event deltas and the internal budget; and the NDJSON codecs with their consumers (the line reader and budget meter, the streaming encoder and sink, validate-once in `CheckpointStore`, and `snapshotEntries` under `TreeTransfer.fromSnapshot`).
...
Step 7's first draft folded the engine's two restore paths, its live image capture and the memoised overlay base into the tree converters, taking the engine from 4,319 to 3,794 lines; the delta draft made a one-file edit in a 50,000-file tree one change of 491 bytes instead of 50,502 records and 9.5 MB, and cut its diff from 325 to 198 ms and its apply from 494 to 99 ms, but serialisation grew from 2,586 to 3,075 lines with the identity walk, the fold's checks and the budget. The codec draft took decode of that tree from 242 to 200 ms, left encode at 205 to 207 ms, and cut peak resident memory while decoding its 14.2 MB encoding from 315 MB to 213 MB collected and 203 MB streamed, the rest being the decoded value; the engine ended at 3,776 lines. Serialisation ended at 3,374 lines, with the line reader and the streaming checks, plus a 146-line snapshot walk, after the draft removed duplicated name, path, payload and order helpers and an unused base64 codec. Two done-when items of issue 185 are not met: serialisation is well over its 1,200-line target, because the Merkle fold, the budget and the streaming reader each added code the old formats did not have, and decode memory is bounded by the budgets plus one line rather than by one record, because the value being built is kept. The larger gain is already in place: a candidate, an overlay, and a snapshot are values of one type, and a delta is a fold over one. That unlocks incremental live commits, overlays on live bases, the three-way merge in issue 174, the subtree-restricted caller in issue 28, and reliable observation revisions in issue 31 as folds and views over the same value. The cost is one coordinated public major and a stacked branch for steps 5 and 6 that must re-prove the staging outcomes before it merges.
