...
3. **Authority equals the path twin.** Each operation requires on the referenced directory or object exactly the mode bits its path-based equivalent requires on the resolved node, the rule the read-only reference operations already follow. Names are single components validated by the adapter; `.` and `..` are rejected before core sees them.
4. **Results carry identity and change.** Creating operations return the new `ObjectReference`. Every namespace mutation returns the affected directory's revision before and after the change; rename returns one pair per directory. Both values are captured inside the single coordination gate hold that performs the mutation, so an adapter may report the pair as atomic. Core does not carry an `atomic` flag, following Buildbarn, because the gate makes every mutation atomic by construction.[^buildbarn][^engine]
5. **Write-open and open-with-create.** `openReference` accepts the existing `OpenSettings` minus `create`, so an existing reference opens for writing, appending, or truncation. A new `openChildReference(directoryReference, nameBytes, OpenSettings)` performs lookup-or-create-and-open in one gate hold and returns the handle, the reference, whether it created the file, and the directory revision pair. The `create: "exclusive" | "ifMissing"` vocabulary is unchanged. Two separate calls would let a concurrent unlink fail the open after the client was told the create succeeded.
6. **Initial timestamps at create.** Create options accept `times` beside `mode`, applied in the creation transition. Core timestamps are bigint nanoseconds, so an adapter can store an eight-byte exclusive-create verifier losslessly in access and modification time, the pattern Linux nfsd uses; an NFS adapter advertises those two attributes in `suppattr_exclcreat`.[^knfsd][^rfc8881] Rejecting exclusive creates, as Buildbarn does, was declined because RFC 8881 Table 18 expects a server without a persistent reply cache to support both exclusive modes.
