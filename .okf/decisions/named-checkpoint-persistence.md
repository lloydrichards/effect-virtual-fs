...
`@effect-vfs/persistence` depends on core; core remains independent of storage I/O. Applications supply the Effect SQL client, database location, and migration timing. The store saves and loads opaque snapshots as existing encoded bytes under create-only names: duplicate saves fail `AlreadyExists`, missing loads fail `NotFound`, and database errors remain distinct. The persistence package does not rewrite snapshot bytes. While snapshot version 1 is being solidified, applications must regenerate checkpoints that use an incompatible earlier schema revision.
