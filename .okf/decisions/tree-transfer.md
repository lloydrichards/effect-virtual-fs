...
An adversarial review of the three delivery pull requests found ways to reach outside a host destination, which a follow-up closed. Escape checks match link names folded, and every created link is also resolved on the host, so name folding and links already in the destination cannot hide an escape. Host files are always created exclusively, so replacing an entry cannot write through a link swapped in after the removal; host replacement is therefore not atomic per file, an accepted exception to per-file atomicity. The hard-link copy fallback applies only when the host cannot link. `toVolume` now applies the metadata defaults, dropping owners and special bits unless requested, so host owner identifiers never become volume owners by accident. Cleanup checks the claimed root's identity and restores owner access before removal, final directory modes are applied uninterruptibly, and every sink rejects the same malformed streams.
