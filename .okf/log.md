...
- **Tree transfer review hardening (#29)**: A four-part adversarial review of the tree transfer pull requests confirmed four ways a host destination could be reached from outside: link-name folding and pre-existing destination links hid escapes, overwrite could write through a link swapped in after removal, and the hard-link fallback copied over colliding names. The follow-up verifies every created link on the host, always creates host files exclusively, narrows the fallback, applies the metadata defaults in `toVolume`, makes cleanup identity-checked and able to remove restrictive directories, and rejects the same malformed streams in every sink. The review also added tests for the high-risk surviving mutations it found; each new attack test fails against the earlier adapter.
