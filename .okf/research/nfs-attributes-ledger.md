...
| owner             | 36 | read-only-local | supported | Decimal uid string without domain                                                               | Same in every profile; Section 5.9 permits numeric strings and Linux and macOS accept them under `sec=sys`. Translation of client-supplied strings arrives with `writable` | #77   |
...
| ------------------------------------------------------ | ---------------------- | --------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| maxfilesize                                            | 27                     | read-only-local | gap(#46)           | Volumes may lower `maxFileBytes`, and the export cannot observe the effective limit; advertising the core ceiling would lie for such volumes[^core] | #46   |
| space_total, space_free, space_avail                   | 44, 43, 42             | read-only-local | gap(#46)           | Truthful only with live usage, which core does not expose; a protocol-neutral usage query is needed first                                           | #46   |
| files_total, files_free, files_avail                   | 23, 22, 21             | read-only-local | gap(#46)           | Same dependency on live entry counts                                                                                                                | #46   |
...
[^dispatcher]: `supportedAttributes`, `encodeAttributeValues`, and `encodeAttributes` define the advertised list and values.
...
[^core]: Core's `maxFileBytes` option is capped at 4294967295 bytes.
