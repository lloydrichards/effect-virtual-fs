...
Watches stream committed create, update, and remove paths through a scoped unbounded buffer. Registration is coordinated with mutations, so a committed event cannot be lost between subscribing and becoming active. Watches have no replay and do not silently drop committed events for nodes a name still resolves. Events are path addressed, so a committed change to a node no name reaches any more, such as a removed directory or an unlinked file still held by an open handle, publishes nothing. Aliases observe changes to the same underlying file. Multi-call adapter helpers are compositions, not transactions.
